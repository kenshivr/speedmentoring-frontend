...
export default function LoginPage({ setUser, setUserId, setSpecialty, setIsAuthenticated }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      //const response = await fetch(`http://localhost:3001/api/login`, { // Para desarrollo local
        method: 'POST',
        headers: {
  ...
  return (
    <div className="container my-5 pt-5 d-flex justify-content-center align-items-center">
      <div className="card shadow-lg" style={{ borderRadius: '25px', width: '100%', maxWidth: '400px', backgroundColor: 'rgba(0, 43, 122, 0.9)' }}>
        <div className="card-body p-4">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              <label htmlFor="exampleFormControlInput1" className="form-label text-white text-center d-block">
                Usuario
              </label>
              <input
                type="text"
            ...
            <div className="mb-3">
              <label htmlFor="inputPassword" className="form-label text-white text-center d-block">
                Contraseña
              </label>
              <input
                type="password"
